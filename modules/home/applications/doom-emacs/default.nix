{ config, lib, pkgs-emacs, pkgs-stable, inputs, ... }:
with lib;
with lib.${namespace};
let
  dashboardLogo = ./cacochan.png;
  cfg = config.${namespace}.applications.doom-emacs;
in {
  options.${namespace}.applications.doom-emacs = {
  ...
  config = mkIf cfg.enable {
    ...
    home.packages = (with pkgs-emacs; [
      ...
      (python3.withPackages
        (p: with p; [ pandas requests epc lxml pysocks pymupdf markdown ]))
    ]) ++ (with pkgs-stable; [ nodejs nodePackages.mermaid-cli ]);
    ...
    home.file.".emacs.d/org-nursery" = { source = "${inputs.org-nursery}"; };

    home.file.".emacs.d/org-krita" = { source = "${inputs.org-krita}"; };
    ...
    home.file.".emacs.d/phscroll" = { source = "${inputs.phscroll}"; };

    home.file.".emacs.d/mini-frame" = { source = "${inputs.mini-frame}"; };
    ...
    #   		(setq user-full-name "'' + userSettings.name + ''
    #     ") ; name
    #       (setq user-username "'' + userSettings.username + ''
    #       ") ; username
    #         (setq user-mail-address "'' + userSettings.email + ''
    #         ") ; email
    #           (setq user-home-directory "/home/'' + userSettings.username + ''
    #           ") ; absolute path to home directory as string
    #             (setq user-default-roam-dir "'' + userSettings.defaultRoamDir
    #   +
    #     ") ; absolute path to home directory as string
    #       (setq system-nix-profile "'' + systemSettings.profile + ''
    #       ") ; what profile am I using?
    #         (setq system-wm-type "'' + userSettings.wmType + ''
    #         ") ; wayland or x11?
    #           (setq doom-font (font-spec :family "'' + userSettings.font + ''
    #           " :size 20)) ; import font
    #             (setq dotfiles-dir "'' + userSettings.dotfilesDir + ''
